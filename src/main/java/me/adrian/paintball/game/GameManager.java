...
public class GameManager {
    ...
    public boolean isAlive(Player player) { return isPlaying(player); }

    // ---------------- PLAYERS ---------------- //
    public void addPlayer(Player player) {
        playerKills.put(player, 0);
        playerCoins.putIfAbsent(player, 0);
    }
    ...
    private void assignTeams() {
        ...
        for (int i = 0; i < players.size(); i++) {
            playerTeams.put(players.get(i), i < half ? GameTeam.BLUE : GameTeam.GREEN);
        }
    }
    ...
    // ---------------- SNOWBALLS ---------------- //
    public boolean canThrowSnowball(Player player) { snowballCount.putIfAbsent(player, 32); return snowballCount.get(player) > 0; }
    public void throwSnowball(Player player) { snowballCount.put(player, snowballCount.get(player) - 1); }
    public void refillSnowballs(Player player, int amount) { snowballCount.put(player, snowballCount.getOrDefault(player, 0) + amount); }
