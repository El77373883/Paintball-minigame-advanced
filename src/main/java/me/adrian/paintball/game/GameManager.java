...
public class GameManager {
    ...
    public boolean isAlive(Player player) {
        return isPlaying(player) && !playerTeams.containsKey(player) == false;
    }
    ...
    public void addCoins(Player player, int amount) {
        playerCoins.put(player, getCoins(player) + amount);
    }

    public void removeCoins(Player player, int amount) {
        playerCoins.put(player, Math.max(0, getCoins(player) - amount));
    }
